...
type IconFinder struct {
	...
	OverrideFinder  func(string) []string
}
...
func (f *IconFinder) FetchIcons(url string) ([]Icon, error) {
	...
	if CacheEnabled() {
		...
		f.icons, err = fetchIcons(url, f.OverrideFinder)
	}
...
func setDefaultHeaders(req *http.Request) {
	req.Header.Set("Accept", "text/html,application/xhtml+xml,application/xml;q=0.9,image/png,image/gif,image/jpg,*/*;q=0.8,application/signed-exchange;v=b3")
	req.Header.Set("User-Agent", "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_14_4) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/75.0.3770.142 Safari/537.36")
